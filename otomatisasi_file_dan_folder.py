...
def create_folder_and_move_file():
    ...
    for i, nama_folder in enumerate(daftar_huruf_kondisi, start=1):
        ...
        for filename in os.listdir(source_path):
            if filename.endswith(f"{nama_folder}.wav"):
                ...
                messages.append(f"File {os.path.basename(destination_file)} dipindahkan ke folder {folder_name}")
    print("Proses pembuatan folder dan pemindahan file selesai.")
    messages.append("Proses pembuatan folder dan pemindahan file selesai.")
    return messages
